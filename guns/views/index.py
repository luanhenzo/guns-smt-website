...
from guns.utils.emails.email_communication import (
    read_template,
...
def index():
    if request.method == "POST":
        ...
        template = read_template(
            os.path.join(TEMPLATES_FOLDER, r"emails\call_us_form.txt")
        )
        credentials = get_email_credentials(
            os.path.join(INSTANCE_FOLDER, r"call_us_mailcred.ini")
        )
        send_email(
            credentials, template, name, email, subject, message, credentials.login
        )
